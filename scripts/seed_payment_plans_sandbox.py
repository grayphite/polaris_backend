"""
Update payment plans with SANDBOX (TEST) Stripe product and price IDs.

This script UPDATES ONLY existing records with SANDBOX mode Stripe IDs.
It will NOT create new records - if records don't exist, it does nothing.

Usage (from repo root):
    python scripts/seed_payment_plans_sandbox.py

What it does:
    1. Checks if Basic plan exists - if not, aborts (no changes)
    2. Updates the plan with SANDBOX Stripe product ID
    3. Checks if basic_monthly price exists - if not, aborts (no changes)
    4. Updates the price with SANDBOX Stripe price IDs (base + overage)

This is an ATOMIC operation - either all updates succeed or nothing changes.

SANDBOX Stripe IDs (configured in this script):
    - Product ID: prod_TJoXEaVfycYbGe
    - Base Price ID: price_1SNAuKAIZic08EhhdUtSPQ1r
    - Overage Price ID: price_1SOEJOAIZic08EhhMj7DdWST

⚠️  WARNING: This script will UPDATE your database with SANDBOX Stripe IDs.
...
# ============================================================================
# SANDBOX STRIPE IDs - Update these if your sandbox IDs change
# ============================================================================
SANDBOX_STRIPE_PRODUCT_ID = "prod_TJoXEaVfycYbGe"
SANDBOX_STRIPE_PRICE_ID = "price_1SNAuKAIZic08EhhdUtSPQ1r"
SANDBOX_STRIPE_OVERAGE_PRICE_ID = "price_1SOEJOAIZic08EhhMj7DdWST"

# Plan configuration (same as live mode, but with sandbox Stripe IDs)
PLAN_CODE = "basic"
...
def update_basic_plan_with_sandbox_ids() -> dict:
    """
    Update existing Basic plan and price with SANDBOX Stripe IDs.
    ...
    if plan.stripe_product_id != SANDBOX_STRIPE_PRODUCT_ID:
        old_product_id = plan.stripe_product_id
        plan.stripe_product_id = SANDBOX_STRIPE_PRODUCT_ID
        plan_updated = True
        print(f"🔄 Updating plan stripe_product_id:")
        print(f"   Old: {old_product_id}")
        print(f"   New: {SANDBOX_STRIPE_PRODUCT_ID}")
    ...
    if price.stripe_price_id != SANDBOX_STRIPE_PRICE_ID:
        old_price_id = price.stripe_price_id
        price.stripe_price_id = SANDBOX_STRIPE_PRICE_ID
        price_updated = True
        print(f"🔄 Updating price stripe_price_id:")
        print(f"   Old: {old_price_id}")
        print(f"   New: {SANDBOX_STRIPE_PRICE_ID}")
    
    # Update stripe_overage_price_id
    if price.stripe_overage_price_id != SANDBOX_STRIPE_OVERAGE_PRICE_ID:
        old_overage_id = price.stripe_overage_price_id
        price.stripe_overage_price_id = SANDBOX_STRIPE_OVERAGE_PRICE_ID
        price_updated = True
        print(f"🔄 Updating price stripe_overage_price_id:")
        print(f"   Old: {old_overage_id}")
        print(f"   New: {SANDBOX_STRIPE_OVERAGE_PRICE_ID}")
...
def main():
    """Main function to update payment plans with sandbox Stripe IDs (UPDATE ONLY - no creates)."""
    print("\n" + "="*80)
    print("UPDATE PAYMENT PLANS - SANDBOX MODE (UPDATE ONLY)")
    print("="*80)
    print("\n⚠️  WARNING: This will UPDATE existing records with SANDBOX Stripe IDs.")
    ...
    print("\nSANDBOX Stripe IDs to be used:")
    print(f"   Product ID: {SANDBOX_STRIPE_PRODUCT_ID}")
    print(f"   Base Price ID: {SANDBOX_STRIPE_PRICE_ID}")
    print(f"   Overage Price ID: {SANDBOX_STRIPE_OVERAGE_PRICE_ID}")
    ...
    with app.app_context():
        try:
            ...
            result = update_basic_plan_with_sandbox_ids()
            ...
            if not result.get('plan_updated') and not result.get('price_updated'):
                ...
                print("="*80)
                print("All Stripe IDs are already set to sandbox values.")
            ...
            print("\n" + "="*80)
            print("✅ SUCCESS - Database updated with SANDBOX Stripe IDs")
            print("="*80)
            print(f"\nPlan Details:")
