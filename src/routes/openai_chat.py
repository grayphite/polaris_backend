"""
OpenAI Chat Routes - HTTP endpoints for OpenAI chat management
...
from src.services.openai_chat_service import openai_chat_service
...
@openai_chat_bp.route('/chats/<int:chat_id>/ai-chats', methods=['GET'])
...
def list_ai_chats(chat_id):
    ...
    # Fetch from service
    result = openai_chat_service.list_ai_chats(
        chat_id=chat_id,
...
@openai_chat_bp.route('/ai-chats/<int:ai_chat_id>', methods=['GET'])
@auth_service.require_auth
@handle_errors
@log_action(ActionType.READ, "ai_chat")
def get_ai_chat(ai_chat_id):
    ...
    # Fetch from service
    ai_chat = openai_chat_service.get_ai_chat_by_id(
...
@openai_chat_bp.route('/chats/<int:chat_id>/ai-chats', methods=['POST'])
@auth_service.require_auth
...
@openai_chat_bp.route('/ai-chats/<int:ai_chat_id>', methods=['PUT'])
@auth_service.require_auth
@validate_request_data(
...
def update_ai_chat(ai_chat_id):
    ...
    old_ai_chat = openai_chat_service.get_ai_chat_by_id(ai_chat_id, current_user.id)
    if not old_ai_chat:
        raise FileNotFoundError("AI chat not found")
    
    # Update via service
    result = openai_chat_service.update_ai_chat(
        ai_chat_id=ai_chat_id,
        user_id=current_user.id,
        data=data
    )
...
@openai_chat_bp.route('/ai-chats/<int:ai_chat_id>', methods=['DELETE'])
@auth_service.require_auth
@handle_errors
...
def delete_ai_chat(ai_chat_id):
    ...
    ai_chat = openai_chat_service.get_ai_chat_by_id(ai_chat_id, current_user.id)
    if not ai_chat:
        raise FileNotFoundError("AI chat not found")
    
    # Delete via service
    result = openai_chat_service.delete_ai_chat(
        ai_chat_id=ai_chat_id,
        user_id=current_user.id
    )
...
@openai_chat_bp.route('/ai-chats/<int:ai_chat_id>/restore', methods=['POST'])
@auth_service.require_auth
@handle_errors
@log_action(ActionType.UPDATE, "ai_chat")
def restore_ai_chat(ai_chat_id):
    ...
    # Restore via service
    result = openai_chat_service.restore_ai_chat(
        ai_chat_id=ai_chat_id,
        user_id=current_user.id
    )
...
@openai_chat_bp.route('/ai-chats/stats', methods=['GET'])
@auth_service.require_auth
@handle_errors
...
def get_ai_chat_stats():
    ...
    stats = openai_chat_service.get_ai_chat_statistics(current_user.id, chat_id)
...
@openai_chat_bp.route('/ai-chats/send-message', methods=['POST'])
@auth_service.require_auth
@validate_request_data(
...
def send_message():
    """
    Send a message to OpenAI and create AI chat record
...
# Health check
@openai_chat_bp.route('/ai-chats/health', methods=['GET'])
def health_check():
    """Health check for OpenAI chat endpoints"""
    try:
        health_status = openai_chat_service.health_check()
