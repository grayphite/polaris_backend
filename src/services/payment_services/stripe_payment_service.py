...
class StripePaymentService:
    ...
    def ensure_basic_plan_catalog() -> Dict[str, Any]:
        ...
        (without committing; caller decides transaction). Uses provided Stripe IDs.
        """
        plan = PaymentPlan.query.filter_by(code='basic').first()
        if not plan:
            plan = PaymentPlan(
                code='basic',
                display_name='Basic',
        ...
        price = PlanPrice.query.filter_by(key='basic_monthly').first()
        if not price:
            price = PlanPrice(
                plan=plan,
                key='basic_monthly',
