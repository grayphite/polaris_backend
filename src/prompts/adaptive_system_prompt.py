...
def get_adaptive_system_prompt(
        ...
        referenced_chat_summaries: list = None
) -> str:
    """
    ...
    base_prompt = (
        ...
        "RESPONSE GUIDELINES:\n"
        "- Provide precise, focused responses with only relevant details—avoid unnecessary elaboration\n"
        ...
        "- Follow the format, style, and structure specified in this system prompt exactly\n"
        ...
        "- Provide precise, well-structured answers that directly address the question\n"
        "- Avoid unnecessary elaboration or irrelevant details\n"
        "- Offer practical insights and examples when relevant\n"
        "- Maintain professional but approachable tone\n"
        "- Focus on accuracy and helpfulness\n\n"
        
        "RESPONSE FORMAT:\n"
        "- Follow the exact format and structure specified in this system prompt\n"
        "- Use plain text only (no markdown, bullet points, or special formatting) unless this system prompt specifies otherwise\n"
        "- Be precise and concise—address the query fully without unnecessary length\n"
        "- Structure responses logically: key findings first, followed by supporting details\n"
        "- Use clear paragraph breaks for readability\n"
        "- Do not add extra sections, disclaimers, or information not requested\n\n"
    ...
    "- Provide precise, focused responses that directly address what is asked—avoid unnecessary elaboration or tangential details.\n"
    "- Follow the format, style, and structure specified in this system prompt exactly.\n"
    ...
    "- Respond professionally, clearly, and concisely—do not add information, sections, or details not explicitly requested.\n"
    ...
    if referenced_chat_summaries:
        ...
        referenced_section += f"The user has referenced {len(referenced_chat_summaries)} chat conversation(s) that contain relevant context for their current question.\n"
        referenced_section += "IMPORTANT: Pay special attention to these referenced chat summaries when answering the user's question.\n"
        referenced_section += "Use information from these referenced chats to provide more informed and contextually relevant responses.\n"
        referenced_section += "If the current question relates to topics discussed in any of the referenced chats, incorporate that context into your answer.\n"
        referenced_section += "When referencing information from a referenced chat, acknowledge it appropriately in your response.\n\n"
        ...
        for i, ref_chat in enumerate(referenced_chat_summaries, 1):
            ...
            referenced_section += f"--- Referenced Chat {i}: {name} (ID: {chat_id}) ---\n"
            referenced_section += f"{summary}\n\n"
